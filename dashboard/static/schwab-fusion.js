(() => {
  ...
  function ensureHost() {
    ...
    host.innerHTML = '<div class="eyebrow">SCHWAB / THINKORSWIM</div><strong>Broker-aware contract fit</strong><div class="fineprint" style="margin-top:5px">Waiting for a MnT analysis.</div>';
  ...
  function renderCandidates(payload, budget) {
    ...
    if (!rows.length) {
      return `<div class="mnt-portfolio-context caution"><span class="mnt-context-label">Schwab contract check</span><strong>NO CONTRACT CLEARED</strong><div class="fineprint" style="margin-top:5px">No Schwab option met the ${money(budget)} budget and current liquidity/fit rules. MnT should not force a cheaper, lower-quality contract.</div></div>`;
    }
    const best = rows[0];
    return `
      <div class="mnt-portfolio-context good">
        ...
        <div class="fineprint" style="margin-top:5px">Research candidate only — no order will be sent.</div>
        <div class="mnt-schwab-fit-grid">
  ...
  async function renderFusion(data) {
    ...
    if (!symbol || !['LONG', 'SHORT'].includes(direction)) {
      host.innerHTML = '<div class="eyebrow">SCHWAB / THINKORSWIM</div><strong>Broker-aware contract fit</strong><div class="fineprint" style="margin-top:5px">MnT needs a LONG or SHORT setup before checking portfolio exposure and contracts.</div>';
      return;
    }

    host.innerHTML = `<div class="eyebrow">SCHWAB / THINKORSWIM</div><strong>${symbol} broker-aware check</strong><div class="fineprint" style="margin-top:5px">Checking your connected positions and Schwab market data…</div>`;
    try {
      const status = await getJson('/api/schwab/status');
      if (!(status.authorized && status.refresh_token_valid)) {
        host.innerHTML = `<div class="eyebrow">SCHWAB / THINKORSWIM</div><strong>Broker awareness not connected</strong><div class="fineprint" style="margin-top:5px">Connect Schwab to make MnT position-aware and use Schwab contracts.</div><div style="margin-top:8px"><a class="ghost-button" href="/broker">Open Broker Desk</a></div>`;
        return;
      }
      ...
      const contextPromise = getJson(`/api/schwab/portfolio/${encodeURIComponent(symbol)}/context?${params}`);
      ...
      let html = `<div class="eyebrow">SCHWAB / THINKORSWIM</div><strong>${symbol} broker-aware check</strong>`;
      ...
      window.dispatchEvent(new CustomEvent('mnt:portfolio-context', { detail: context }));
    } catch (error) {
      host.innerHTML = `<div class="eyebrow">SCHWAB / THINKORSWIM</div><strong>Broker-aware check unavailable</strong><div class="fineprint" style="margin-top:5px">${safe(error.message)}</div>`;
    }
  }
